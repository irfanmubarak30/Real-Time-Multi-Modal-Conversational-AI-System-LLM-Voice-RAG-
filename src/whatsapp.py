...
class WhatsAppBot:
    ...
    def __init__(self):
        ...
        # Fees images configuration - Add your fees-related images here
        self.fees_images = [
            {
                'url': 'https://drive.google.com/uc?export=download&id=YOUR_FEES_IMAGE_1_ID',
                'caption': '💰 Course Fee Structure & Payment Plans'
            },
            {
                'url': 'https://drive.google.com/uc?export=download&id=YOUR_FEES_IMAGE_2_ID',
                'caption': '💳 Flexible Payment Options Available'
            },
            {
                'url': 'https://drive.google.com/uc?export=download&id=YOUR_FEES_IMAGE_3_ID',
                'caption': '🎓 Scholarship & Discount Programs'
            },
            {
                'url': 'https://drive.google.com/uc?export=download&id=YOUR_FEES_IMAGE_4_ID',
                'caption': '📊 Course-wise Fee Breakdown'
            },
            {
                'url': 'https://drive.google.com/uc?export=download&id=YOUR_FEES_IMAGE_5_ID',
                'caption': '💸 EMI & Installment Options'
            }
        ]
        ...
        # Test all placement/fees media URLs on startup
        self._test_placement_image_urls()
        ...
        # Keywords that trigger fees images (English + Malayalam)
        self.fees_keywords = [
            # English keywords
            'fees', 'fee', 'cost', 'price', 'amount', 'money', 'payment', 'pay',
            'how much', 'course fee', 'fees structure', 'fee structure', 'pricing',
            'course cost', 'course price', 'tuition', 'tuition fee', 'admission fee',
            'total cost', 'total fee', 'course amount', 'what is the fee',
            'what is the cost', 'how much does it cost', 'how much fee',
            'payment plan', 'installment', 'emi', 'scholarship', 'discount',
            'affordable', 'cheap', 'expensive', 'budget', 'financial',
            'fee details', 'cost details', 'price details', 'payment options',
            
            # Malayalam keywords
            'ഫീസ്', 'ഫീ', 'കോസ്റ്റ്', 'വില', 'കൂലി', 'പൈസ', 'പേയ്‌മെന്റ്', 'കൊടുക്കുക',
            'എത്ര കൂലി', 'കോഴ്സ് ഫീസ്', 'ഫീസ് ഘടന', 'വിലയും',
            'കോഴ്സ് കോസ്റ്റ്', 'കോഴ്സ് വില', 'ട്യൂഷൻ ഫീസ്', 'അഡ്മിഷൻ ഫീസ്',
            'മൊത്തം കോസ്റ്റ്', 'മൊത്തം ഫീസ്', 'കോഴ്സ് അമൗണ്ട്',
            'ഫീസ് എന്താണ്', 'കോസ്റ്റ് എന്താണ്', 'എത്ര കോസ്റ്റ്', 'എത്ര ഫീസ്',
            'പേയ്‌മെന്റ് പ്ലാൻ', 'ഇൻസ്റ്റാൾമെന്റ്', 'emi', 'സ്കോളർഷിപ്', 'ഡിസ്കൗണ്ട്',
            'താങ്ങാനാവുന്ന', 'കുറഞ്ഞ', 'വിലയേറിയ', 'ബഡ്ജറ്റ്', 'സാമ്പത്തിക',
            'ഫീസ് വിവരങ്ങൾ', 'കോസ്റ്റ് വിവരങ്ങൾ', 'വില വിവരങ്ങൾ', 'പേയ്‌മെന്റ് ഓപ്ഷനുകൾ'
        ]
    ...
    def _test_placement_image_urls(self):
        """Validate placement and fees image URLs at startup and log results.
        This prevents runtime surprises and helps detect expired Google Drive links.
        """
        try:
            # Helper to check a single URL
            def check_url(url: str) -> tuple[bool, int | None, str | None]:
                try:
                    # Prefer HEAD to be lightweight; fall back to GET if not allowed
                    resp = requests.head(url, allow_redirects=True, timeout=8)
                    if resp.status_code in (405, 403) or resp.headers.get('Content-Length') is None:
                        resp = requests.get(url, allow_redirects=True, timeout=12, stream=True)
                    return (200 <= resp.status_code < 400, resp.status_code, None)
                except Exception as e:
                    return (False, None, str(e))

            # Check placement images
            for idx, item in enumerate(getattr(self, 'placement_images', []), start=1):
                ok, code, err = check_url(item.get('url', ''))
                if ok:
                    logger.info(f"✅ Placement image {idx} URL OK (status {code})")
                else:
                    logger.warning(f"⚠️ Placement image {idx} URL may be invalid (status {code}, error {err}) — {item.get('url')}")

            # Check fees images
            for idx, item in enumerate(getattr(self, 'fees_images', []), start=1):
                ok, code, err = check_url(item.get('url', ''))
                if ok:
                    logger.info(f"✅ Fees image {idx} URL OK (status {code})")
                else:
                    logger.warning(f"⚠️ Fees image {idx} URL may be invalid (status {code}, error {err}) — {item.get('url')}")

            logger.info("🔎 Media URL validation completed")
        except Exception as e:
            logger.error(f"Error while testing media URLs: {e}")
    ...
    def detect_flutter_intent(self, message):
        ...
        for keyword in self.flutter_keywords:
            if keyword in message_lower:
                logger.info(f"🚀 Flutter intent detected with keyword: {keyword}")
                return True
        
        # Check for common Flutter phrases
        flutter_phrases = [
        ...
        for phrase in flutter_phrases:
            if phrase in message_lower:
                logger.info(f"🚀 Flutter intent detected with phrase: {phrase}")
                return True
        
        return False
    
    def detect_fees_intent(self, message):
        """Detect if user is asking about fees/cost-related queries"""
        message_lower = message.lower()
        
        # Check for fees keywords
        for keyword in self.fees_keywords:
            if keyword in message_lower:
                logger.info(f"💰 Fees intent detected with keyword: {keyword}")
                return True
        
        # Check for common fees phrases
        fees_phrases = [
        ...
        for phrase in fees_phrases:
            if phrase in message_lower:
                logger.info(f"💰 Fees intent detected with phrase: {phrase}")
                return True
        
        return False
    
    def send_placement_images_carousel(self, to_phone):
        """Send placement images as a WhatsApp carousel using Twilio Content API with multilingual support"""
        try:
            # Get user's preferred language
            user_session = self.user_sessions.get(to_phone, {})
            preferred_language = user_session.get('preferred_language', 'english')
            is_malayalam = preferred_language == 'malayalam'
            
            # Multilingual text content
            carousel_texts = {
                'body': {
                    'english': "🎯 Our Complete Placement Success Story & Job Assurance Program",
                    'malayalam': "🎯 ഞങ്ങളുടെ സമ്പൂർണ്ണ പ്ലേസ്‌മെന്റ് വിജയകഥയും ജോബ് അഷ്വറൻസ് പ്രോഗ്രാമും"
                },
                'title': {
                    'english': "Placement Success #{}",
                    'malayalam': "പ്ലേസ്‌മെന്റ് വിജയം #{}"
                },
                'button1': {
                    'english': "Tell me more!",
                    'malayalam': "കൂടുതൽ പറയൂ!"
                },
                'button2': {
                    'english': "Contact counselor",
                    'malayalam': "കൗൺസിലറെ ബന്ധപ്പെടുക"
                },
                'follow_up': {
                    'english': """💬 Swipe through our placement success stories above!
📞 Want to speak with our placement counselor? Just tap the button!
🎯 Ready to start your journey? Type 'placement details' for more info!""",
                    'malayalam': """💬 മുകളിലുള്ള ഞങ്ങളുടെ പ്ലേസ്‌മെന്റ് വിജയകഥകൾ സ്വൈപ്പ് ചെയ്യുക!
📞 ഞങ്ങളുടെ പ്ലേസ്‌മെന്റ് കൗൺസിലറുമായി സംസാരിക്കാൻ ആഗ്രഹിക്കുന്നുണ്ടോ? ബട്ടൺ ടാപ്പ് ചെയ്യുക!
🎯 നിങ്ങളുടെ യാത്ര ആരംഭിക്കാൻ തയ്യാറാണോ? കൂടുതൽ വിവരങ്ങൾക്ക് 'placement details' ടൈപ്പ് ചെയ്യുക!"""
                }
            }
            
            lang = 'malayalam' if is_malayalam else 'english'
            
            # Create carousel content template
            carousel_data = {
                "friendly_name": f"placement_carousel_{int(time.time())}",
                "language": "en",  # Twilio language code (always 'en' for now)
                "types": {
                    "twilio/carousel": {
                        "body": carousel_texts['body'][lang],
                        "cards": []
                    }
                }
            }
            
            # Convert placement images to carousel cards
            logger.info(f"🔍 Processing {len(self.placement_images)} placement images for carousel")
            for i, image_data in enumerate(self.placement_images, 1):
                card = {
                    "title": carousel_texts['title'][lang].format(i),
                    "body": image_data['caption'][:160],  # WhatsApp has character limits
                    "media": image_data['url'],
                    "actions": [
                        {
                            "type": "QUICK_REPLY",
                            "title": carousel_texts['button1'][lang],
                            "id": f"placement_info_{i}"
                        },
                        {
                            "type": "QUICK_REPLY", 
                            "title": carousel_texts['button2'][lang],
                            "id": "contact_counselor"
                        }
                    ]
                }
                carousel_data["types"]["twilio/carousel"]["cards"].append(card)
                logger.info(f"✅ Added card {i}: {image_data['caption'][:50]}... (URL: {image_data['url'][:50]}...)")
            
            logger.info(f"📊 Total carousel cards created: {len(carousel_data['types']['twilio/carousel']['cards'])}")
            
            # Debug: Print the full carousel data structure
            logger.info(f"🔍 DEBUG: Carousel data structure: {json.dumps(carousel_data, indent=2)}")
            
                        # Create content template using direct REST API
            import base64
            ...
            if response.status_code == 201:
                ...
                content_sid = content_response['sid']
                logger.info(f"✅ Created carousel content template: {content_sid} (Language: {lang})")
            else:
                logger.error(f"❌ Failed to create content template: {response.status_code} - {response.text}")
                return False
            ...
            logger.info(f"✅ Sent placement carousel to {to_phone}: {message.sid}")
            
            # Send follow-up message in user's language
            self.send_whatsapp_message(to_phone, carousel_texts['follow_up'][lang])
            
            return True
            
        except Exception as e:
            logger.error(f"❌ Error sending placement carousel to {to_phone}: {e}")
            return False

    def send_fees_images_carousel(self, to_phone):
        """Send fees images as a WhatsApp carousel using Twilio Content API with multilingual support"""
        try:
            # Get user's preferred language
            user_session = self.user_sessions.get(to_phone, {})
            preferred_language = user_session.get('preferred_language', 'english')
            ...
            carousel_texts = {
                'body': {
                    'english': "💰 Complete Fee Structure & Payment Options",
                    'malayalam': "💰 സമ്പൂർണ്ണ ഫീസ് ഘടനയും പേയ്‌മെന്റ് ഓപ്ഷനുകളും"
                },
                'title': {
                    'english': "Fee Details #{}",
                    'malayalam': "ഫീസ് വിവരങ്ങൾ #{}"
                },
                'button1': {
                    'english': "More details!",
                    'malayalam': "കൂടുതൽ വിവരങ്ങൾ!"
                },
                'button2': {
                    'english': "Payment options",
                    'malayalam': "പേയ്‌മെന്റ് ഓപ്ഷനുകൾ"
                },
                'follow_up': {
                    'english': """💳 Swipe through our complete fee structure above!
💰 Need help with payment plans? Just tap the button!
🎓 Ready to enroll? Type 'admission' for next steps!""",
                    'malayalam': """💳 മുകളിലുള്ള ഞങ്ങളുടെ സമ്പൂർണ്ണ ഫീസ് ഘടന സ്വൈപ്പ് ചെയ്യുക!
💰 പേയ്‌മെന്റ് പ്ലാനുകളിൽ സഹായം വേണോ? ബട്ടൺ ടാപ്പ് ചെയ്യുക!
🎓 അഡ്മിഷൻ എടുക്കാൻ തയ്യാറാണോ? അടുത്ത സ്റ്റെപ്പുകൾക്ക് 'admission' ടൈപ്പ് ചെയ്യുക!"""
                }
            }
            ...
            # Create carousel content template
            carousel_data = {
                "friendly_name": f"fees_carousel_{int(time.time())}",
                "language": "en",  # Twilio language code (always 'en' for now)
                "types": {
                    "twilio/carousel": {
                        "body": carousel_texts['body'][lang],
            ...
            # Convert fees images to carousel cards
            ...
            logger.info(f"✅ Sent fees carousel to {to_phone}: {message.sid}")
            ...
            logger.error(f"❌ Error sending fees carousel to {to_phone}: {e}")
            return False

    def handle_carousel_response(self, button_payload, from_number):
        """Handle responses from carousel button clicks with multilingual support"""
        try:
            # Get user's preferred language
            ...
            if button_payload.startswith('placement_info_'):
                # Extract placement image number
                image_num = button_payload.split('_')[-1]
                
                if is_malayalam:
                    response = f"""
📸 **പ്ലേസ്‌മെന്റ് വിജയകഥ #{image_num}**

🎯 ഇത് ഞങ്ങളുടെ നിരവധി വിജയകരമായ പ്ലേസ്‌മെന്റ് കഥകളിൽ ഒന്നാണ്! ഞങ്ങളുടെ വിദ്യാർത്ഥികൾ മികച്ച പാക്കേജുകളുമായി മുൻനിര കമ്പനികളിൽ സ്ഥിരമായി പ്ലേസ്‌മെന്റ് നേടുന്നു.

💼 **ഞങ്ങളുടെ പ്ലേസ്‌മെന്റുകളെ പ്രത്യേകമാക്കുന്നത്:**
• 100% ജോബ് അഷ്വറൻസ് പ്രോഗ്രാം
• ഇൻഡസ്ട്രി-പ്രസക്തമായ കരിക്കുലം
• സമർപ്പിത പ്ലേസ്‌മെന്റ് സപ്പോർട്ട്
• മോക്ക് ഇന്റർവ്യൂകളും റെസ്യൂം ബിൽഡിംഗും
• നേരിട്ടുള്ള കമ്പനി ടൈ-അപ്പുകൾ

📞 ഞങ്ങളുടെ പ്ലേസ്‌മെന്റ് ഗ്യാരണ്ടിയെക്കുറിച്ച് കൂടുതൽ അറിയാൻ ആഗ്രഹിക്കുന്നുണ്ടോ? 'placement details' ടൈപ്പ് ചെയ്യുക അല്ലെങ്കിൽ വ്യക്തിഗത കൺസൾട്ടേഷനുവേണ്ടി 'കൗൺസിലറെ ബന്ധപ്പെടുക' ക്ലിക്ക് ചെയ്യുക!
                    """
                else:
                    response = f"""
📸 **Placement Success Story #{image_num}**

🎯 This showcases one of our many successful placement stories! Our students consistently get placed in top companies with excellent packages.

💼 **What makes our placements special:**
...
📞 Want to know more about our placement guarantee? Type 'placement details' or click 'Contact counselor' for a personal consultation!
                    """
                
                self.send_whatsapp_message(from_number, response.strip())
                return True
                
            elif button_payload == 'contact_counselor':
                if is_malayalam:
                    response = """
📞 **ഞങ്ങളുടെ പ്ലേസ്‌മെന്റ് കൗൺസിലറുമായി ബന്ധപ്പെടുക**

ഞങ്ങളുടെ പ്ലേസ്‌മെന്റ് വിദഗ്ധർ നിങ്ങളെ സഹായിക്കാൻ തയ്യാറാണ്! എങ്ങനെ ബന്ധപ്പെടാം:

🔸 **നേരിട്ട് വിളിക്കുക:** +91-XXXXXXXXXX (നിങ്ങളുടെ നമ്പർ ഇവിടെ ചേർക്കുക)
🔸 **WhatsApp:** തൽക്ഷണ സപ്പോർട്ടിനായി ഇവിടെ ചാറ്റ് തുടരുക
🔸 **കോൾ ഷെഡ്യൂൾ ചെയ്യുക:** സൗകര്യപ്രദമായ സമയം ബുക്ക് ചെയ്യാൻ 'schedule call' ടൈപ്പ് ചെയ്യുക

💬 **അല്ലെങ്കിൽ ഇവയെക്കുറിച്ച് എന്തും ചോദിക്കുക:**
• കോഴ്സ് വിശദാംശങ്ങളും കരിക്കുലവും
• പ്ലേസ്‌മെന്റ് സ്ഥിതിവിവരക്കണക്കുകളും കമ്പനികളും
• ഫീസ് ഘടനയും പേയ്‌മെന്റ് ഓപ്ഷനുകളും
• ബാച്ച് സമയവും കാലാവധിയും

നിങ്ങൾക്ക് എന്തിനെക്കുറിച്ചാണ് കൂടുതൽ അറിയാൻ താൽപ്പര്യം?
                    """
                else:
                    response = """
📞 **Connect with Our Placement Counselor**

Our placement experts are ready to help you! Here's how to connect:

🔸 **Call directly:** +91-XXXXXXXXXX (Replace with your number)
🔸 **WhatsApp:** Continue chatting here for instant support
🔸 **Schedule a call:** Type 'schedule call' to book a convenient time
...
What would you like to know more about?
            ...
            elif button_payload.startswith('fees_info_'):
                # Extract fees image number
                image_num = button_payload.split('_')[-1]
                
                if is_malayalam:
                    response = f"""
💰 **ഫീസ് വിവരങ്ങൾ #{image_num}**

💳 ഇത് ഞങ്ങളുടെ കോഴ്സ് ഫീസ് ഘടനയുടെ വിശദാംശങ്ങളാണ്! ഞങ്ങൾ വിദ്യാർത്ഥികൾക്ക് സൗകര്യപ്രദവും താങ്ങാനാവുന്നതുമായ പേയ്‌മെന്റ് ഓപ്ഷനുകൾ നൽകുന്നു.

💼 **ഞങ്ങളുടെ ഫീസ് ഘടനയുടെ പ്രത്യേകതകൾ:**
• സുതാര്യമായ ഫീസ് ഘടന
• ഫ്ലെക്സിബിൾ പേയ്‌മെന്റ് പ്ലാനുകൾ
• EMI & ഇൻസ്റ്റാൾമെന്റ് ഓപ്ഷനുകൾ
• സ്കോളർഷിപ്പ് & ഡിസ്കൗണ്ട് പ്രോഗ്രാമുകൾ
• മറഞ്ഞിരിക്കുന്ന ചാർജുകൾ ഇല്ല

💰 പേയ്‌മെന്റ് പ്ലാനുകളെക്കുറിച്ച് കൂടുതൽ അറിയാൻ 'payment options' ക്ലിക്ക് ചെയ്യുക അല്ലെങ്കിൽ 'fees details' ടൈപ്പ് ചെയ്യുക!
                    """
                else:
                    response = f"""
💰 **Fee Details #{image_num}**

💳 This shows our detailed course fee structure! We provide flexible and affordable payment options for all students.

💼 **What makes our fee structure special:**
...
💰 Want to know more about payment plans? Click 'Payment options' or type 'fees details' for more info!
                    """
                
                self.send_whatsapp_message(from_number, response.strip())
                return True
                
            elif button_payload == 'payment_options':
                if is_malayalam:
                    response = """
💳 **പേയ്‌മെന്റ് ഓപ്ഷനുകളും ഫ്ലെക്സിബിൾ പ്ലാനുകളും**

ഞങ്ങൾ നിങ്ങളുടെ സൗകര്യത്തിനായി വിവിധ പേയ്‌മെന്റ് ഓപ്ഷനുകൾ നൽകുന്നു:

💰 **പേയ്‌മെന്റ് ഓപ്ഷനുകൾ:**
• ഒറ്റത്തവണ പേയ്‌മെന്റ് (അധിക ഡിസ്കൗണ്ട്)
• 2-3 ഇൻസ്റ്റാൾമെന്റുകൾ
• EMI ഓപ്ഷനുകൾ (6-12 മാസം)
• സ്കോളർഷിപ് പ്രോഗ്രാമുകൾ

🎓 **പ്രത്യേക ഓഫറുകൾ:**
• നേരത്തെ രജിസ്റ്റർ ചെയ്യുന്നവർക്ക് ഡിസ്കൗണ്ട്
• ഗ്രൂപ്പ് ഡിസ്കൗണ്ട് (3+ വിദ്യാർത്ഥികൾ)
• റെഫറൽ ബോണസ്

💬 കൂടുതൽ വിവരങ്ങൾക്ക് 'admission' ടൈപ്പ് ചെയ്യുക അല്ലെങ്കിൽ ഞങ്ങളുടെ ടീമുമായി ബന്ധപ്പെടുക!
                    """
                else:
                    response = """
💳 **Payment Options & Flexible Plans**

We offer various payment options for your convenience:

💰 **Payment Options:**
• One-time payment (extra discount)
• 2-3 installments
• EMI options (6-12 months)
• Scholarship programs

🎓 **Special Offers:**
• Early bird discount
• Group discount (3+ students)
• Referral bonus

💬 For more details, type 'admission' or contact our team!
                    """
            ...
            logger.error(f"❌ Error handling carousel response: {e}")
            return False
        ...
        for phone, session in list(self.user_sessions.items()):
            try:
                ...
                if inactive_minutes >= timeout_minutes:
                    ...
                    # Analyze conversation and save interest score before ending session
                    try:
                        chatbot = session.get('chatbot')
                        if chatbot and hasattr(chatbot, 'end_session_with_analysis'):
                            logger.info(f"🔍 Analyzing conversation for {phone} before session end...")
                            analysis_result = chatbot.end_session_with_analysis()
                            if analysis_result:
                                logger.info(f"✅ Conversation analysis completed for {phone}: Score {analysis_result['interest_score']}/100")
                                logger.info(f"📝 Summary: {analysis_result['summary'][:100]}...")
                            else:
                                logger.warning(f"⚠️ Conversation analysis failed for {phone}")
                        else:
                            logger.warning(f"⚠️ No chatbot instance found for conversation analysis for {phone}")
                    except Exception as e:
                        logger.error(f"❌ Error during conversation analysis for {phone}: {e}")
                ...
                # Safety check: If user is on Q4 (final question), they've completed the survey
                if not survey_completed and current_survey_q == 'q4':
                    logger.info(f"🔧 SAFETY FIX: Phone {phone} - User on Q4, setting survey_completed = True")
...
def whatsapp_webhook():
    ...
    try:
        ...
        # Check for carousel button responses
        button_payload = request.form.get('ButtonPayload', '')
        if button_payload:
            logger.info(f"📱 Received carousel button response from {from_number}: {button_payload}")
            if whatsapp_bot.handle_carousel_response(button_payload, from_number):
                # Button response handled successfully
            ...
            if not user_session['language_selected']:
                ...
                # Check if user is in survey mode
                    ...
                    if user_session.get('awaiting_q1_other'):
                        ...
                        # Process survey poll response
                        poll_result = whatsapp_bot.process_poll_response(message_body, user_session)
                        if poll_result["type"] == "error":
                            response_text = poll_result["text"]
                        elif poll_result["type"] == "followup":
                            # Send follow-up message immediately
                            whatsapp_bot.send_whatsapp_message(from_number, poll_result["text"])
                            # If there is a next question, send it as a separate WhatsApp message
                            if poll_result["next"] in ['q2', 'q3', 'q4']:
                                whatsapp_bot.send_survey_poll(from_number, poll_result["next"])
                            elif poll_result["next"] == 'complete':
                                user_session['survey_completed'] = True
                                # The chatbot is now in COLLECTING_NAME status
                                if user_session.get('preferred_language') == 'malayalam':
                                    whatsapp_bot.send_whatsapp_message(from_number, "🎉 ഞങ്ങളുടെ വ്യക്തിഗത സർവേ പൂർത്തിയാക്കിയതിന് നന്ദി! നിങ്ങളുടെ മറുപടികൾ അടിസ്ഥാനമാക്കി, നിങ്ങളുക്കായി ഒരു ഇഷ്ടാനുസൃത പഠന പാത സൃഷ്ടിക്കും.\n\nഇപ്പോൾ, ഞങ്ങളുടെ ടീമുമായി നിങ്ങളെ ബന്ധിപ്പിക്കാനും നിങ്ങളുടെ വ്യക്തിഗത ശുപാർശകൾ അയയ്ക്കാനും, നിങ്ങളുടെ പൂർണ്ണ നാമം പങ്കുവെയ്ക്കാമോ?")
                                else:
                                    whatsapp_bot.send_whatsapp_message(from_number, "🎉 Thank you for completing our personalization survey! Based on your responses, we'll create a customized learning path just for you.\n\nNow, to connect you with our team and send your personalized recommendations, could you please share your full name?")
                            response_text = None  # No TwiML response needed, already sent
                ...
                if whatsapp_bot.detect_placement_intent(message_body) and not user_session.get('placement_images_sent', False):
                    if whatsapp_bot.send_placement_images_carousel(from_number):
                ...
                # Check for fees-related queries
                elif whatsapp_bot.detect_fees_intent(message_body) and not user_session.get('fees_images_sent', False):
                    if whatsapp_bot.send_fees_images_carousel(from_number):
                        user_session['fees_images_sent'] = True
                        response_text = None  # No additional response needed
                    else:
                        response_text = "I'm having trouble loading the fee information. Please try again later."
        ...
        if response_text:
            ...
            # Send welcome GIF after successful text message (only for first-time users)
            if not user_session.get('welcome_sent', False) and not user_session['language_selected']:
...
def send_placement_images_manual():
    ...
    try:
        ...
        success = whatsapp_bot.send_placement_images_carousel(phone_number)
...
def send_fees_images_manual():
    """Manual endpoint to send fees images carousel (for testing)"""
    try:
        data = request.json
        phone_number = data.get('phone')
        ...
        success = whatsapp_bot.send_fees_images_carousel(phone_number)
        
        if success:
            return jsonify({'status': 'sent', 'phone': phone_number, 'images_count': len(whatsapp_bot.fees_images)})
        else:
            return jsonify({'error': 'Failed to send fees images carousel'}), 500
